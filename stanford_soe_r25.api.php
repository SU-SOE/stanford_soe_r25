...
// examples of calls to stanford_soe_r25 hooks
 ...
 */
function hook_stanford_soe_r25_contact_alter(&$contact_string) {
 ...
 * 
 */
function hook_stanford_soe_r25_fullcalendar_limit_alter(&$calendar_limit) {
 ...
 */
function hook_stanford_soe_r25_isbillable_alter(&$is_billable) {
  if ($is_billable) {
 ...
 */
function hook_stanford_soe_r25_external_link($link_text = 'Authenticate') {
 ...
 *
 */
function hook_stanford_soe_r25_external_user() {
  $return_data = false;
  if (!empty($_COOKIE['user0_webauth'])) {
    ...
    if (empty($user0_info) || empty($user0_info['R25_EXTERNAL_UID'])) {
      watchdog('stanford_soe_r25','External login failed. Missing data file or uid.');
      @unlink($filename);
    } else {
 ...
 *   the user array created by hook_stanford_soe_r25_external_user
 *
 * @return array
 *   a string formatted from information in the array
 *
 */
function hook_stanford_soe_r25_external_user_display($user_info) {
  $contact = '';
